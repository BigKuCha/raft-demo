...
func NewRaft() *Mraft {
	if !flag.Parsed() {
		fmt.Println("flag not parse")
		flag.Parse()
	}
	log.Printf("!!!!%+v", conf.Conf)
	cfg := raft.DefaultConfig()
	cfg.LocalID = raft.ServerID(conf.Conf.Raft.LocalID)
	// 新建内存日志存储
	logstore := raft.NewInmemStore()
	address := conf.Conf.Raft.Addr
	// 节点间通讯地址
	...
	return &Mraft{
		Raft: r,
		Meta: map[string]string{
			"http_port": conf.Conf.App.HttpAddr,
		},
	}
